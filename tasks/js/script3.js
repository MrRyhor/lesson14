...
const totalAmount = dataList.reduce((prevSum, element) => prevSum + element.price, 0)
document.write(`<p>1) Загальну вартість (нові ціни - price) = <span>${totalAmount}</span></p><br>`)

//========================== 2 Знайти кількість товарів, у яких ціна зменшилась (price < old_price). ==========================================================
const qntItemsWithPriceLessThanOldPriace = dataList.reduce((prevQnt, element) => element.price < element.old_price ? prevQnt + 1 : prevQnt, 0)
document.write(`<p>2) Знайти кількість товарів, у яких ціна зменшилась (price < old_price) = <span>${qntItemsWithPriceLessThanOldPriace}</span></p><br>`)

//========================== 3 Товари, які доступні (sell_status:"available") ==========================================================
const itemsAvailable = dataList.filter(element => element.sell_status === "available")
console.log(itemsAvailable)
document.write(`<p>3)Товари, які доступні (sell_status:"available") = <span>См. в консоль</span></p><br>`)

//===================== 4 сформувати новий список об”єктів тільки доступних для продажу товарів, які міститимуть===========================================================================
//============ тільки ідентифікатор товару(id), нову ціну(price), стару ціну(old_price), та ціну у доларах(usd_price) ==========================================================
const newListWithAvailableItems = itemsAvailable.map(element => { return { id: element.id, price: element.price, old_price: element.old_price, usd_price: element.usd_price } })
console.log(newListWithAvailableItems)
document.write(`<p>4) New Array = <span>См. в консоль</span></p><br>`)
